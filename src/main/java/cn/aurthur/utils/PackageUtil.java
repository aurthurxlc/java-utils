...
public abstract class PackageUtil {
    ...
    public static List<String> getResourceInPackage(String packageName) throws IOException {
        ...
        String packagePath = __getPackagePath(packageName);
        ...
        for (URL url : dirs) {
            ...
            if ("file".equals(protocol)) {
                __findResourceInDirPackage(packagePath, URLDecoder.decode(url.getFile(), "UTF-8"), resources);
            } else if ("jar".equals(protocol)) {
                __findResourceInJarPackage(url, packageName, packageDirName, recursive, resources);
            }
    ...
    public static List<String> getClassesInPackage(String packageName, List<String> included, List<String> excluded)
        ...
        for (URL url : dirs) {
            String protocol = url.getProtocol();
            if ("file".equals(protocol)) {
                __findClassesInDirPackage(packagePath, included, excluded, URLDecoder.decode(url.getFile(), "UTF-8"),
                        recursive, classes);
            } else if ("jar".equals(protocol)) {
                __findClassesInJarPackage(url, packageName, included, excluded, packageDirName, recursive, classes);
            }
        }
        return classes;
    }
            ...
            if (name.startsWith(packageDirName)) {
                ...
                if ((idx != -1) || recursive) {
                    ...
                    if (name.endsWith(".class") && !entry.isDirectory()) {
                        ...
                        __filterClass(packageName, className, included, excluded, classes);
                    }
                }
            }
        }
    ...
    private static void __findClassesInDirPackage(String packageName, List<String> included, List<String> excluded,
        ...
        for (File file : dirfiles) {
            ...
            __filterClass(packageName, file.getName().substring(0, file.getName().length() - 6), included, excluded,
                    classes);
     ...
     * @param resources   存储资源的列表
     */
    private static void __findResourceInDirPackage(String packageName, String packagePath, List<String> resources) {
        File dir = new File(packagePath);
        if (!dir.exists() || !dir.isDirectory()) {
            return;
        }
        ...
        for (File file : dirfiles) {
            if (file.isDirectory()) {
                __findResourceInDirPackage(packageName + "." + file.getName(), file.getAbsolutePath(), resources);
                continue;
            }
     ...
     * @return 包路径
     */
    private static String __getPackagePath(String packageName) {
     ...
     */
    private static boolean __isIncluded(String name, List<String> included, List<String> excluded) {
        if (EmptyUtil.isEmpty(included) && EmptyUtil.isEmpty(excluded)) {
            return true;
        }

        included = (null == included) ? EMPTY_LIST : included;
        excluded = (null == excluded) ? EMPTY_LIST : excluded;
        boolean isIncluded = PackageUtil.__isMatched(name, included);
        boolean isExcluded = PackageUtil.__isMatched(name, excluded);
     ...
     */
    private static boolean __isMatched(String name, List<String> list) {
